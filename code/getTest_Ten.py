...
def main(_):
  with tf.Graph().as_default():
    ...
    for path in testPathes:
        ...
        ss2 = ss[:,1:].max(axis=1)
        ssID = np.argmax(ss2)
        sID = np.argmax(ss[ssID,:])
        if sID==0:label = 'norm'
        else :label = 'defect_'+str(sID)
        singg = os.path.basename(path)+'|'+'norm'+','+"%.5f"%min(ss[ssID,0],0.99999)
        print(singg)
        li.append(singg)
        for i in range(1,11):
          singg = os.path.basename(path)+'|'+'defect_'+str(i)+','+'%.5f'%min(ss[ssID,i],0.99999)
          li.append(singg)
